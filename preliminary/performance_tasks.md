...
4. In order to complete the project, we need to install OpenCV. Fill in the following:
  ...
  > OpenCV (Open Source Computer Vision Library) is an open-source software toolkit for computer vision and machine learning tasks. 
...
6. Have the dependencies in the `pyproject.toml` changed? If so, how?
  > yes, dependencies list has been updated to inckude "opencv-python>=4.13.0.92"
7. Why did we use `uv add` over `uv pip`?
  > uv add automatically updates the pyproject.toml and generates a uv.lock file, keeping the environment reproducible and shareable across teams. uv pip install only installs the package into your current environment.
8. The `numpy` library is required for OpenCV. Should you add an explicit requirement for it? Why/Why not?
  > No, it is a dependency of OpenCV and will be automatically installed when we install OpenCV. Adding it explicitly could lead to version conflicts if OpenCV requires a specific version of numpy.
...
> how long ago was a commit made to the library -> [Feb 18, 2025](https://github.com/madmaze/pytesseract/commits/master/) ;
> does it have external dependencies -> It does "[Pillow](https://pypi.org/project/pillow/)" ;
> how does it suite the project requirements -> it will recognize and "read" the text embedded in images .
